...
plt.legend()
plt.grid()
plt.savefig("plots/residualsEp"+label+".png")
...
plt.title("pulls")
plt.grid()
plt.savefig("plots/residualsR"+label+".png")
...
plt.title("pulls")
plt.grid()
plt.savefig("plots/residualsTime"+label+".png")
...
plt.legend()
plt.grid()
plt.savefig("plots/roc_auc_"+label+".png")
...
plt.legend()
plt.grid()
plt.savefig("plots/finalCost_"+label+".png")
...
plt.subplot(1, 2, 1)
plt.hist2d(pt, Ep, range=[[22,28],[0.5,1.5]], bins=50, cmap="hot_r")
...
plt.subplot(1, 2, 2)
plt.hist2d(np.abs(eta), Ep, bins=50, range=[[1.6, 2.9], [0, 2]], cmap="hot_r")
plt.xlabel(r"$\eta$")
plt.ylabel("E/p")
plt.title("E/p for HAD objects")
plt.grid()
plt.savefig("plots/f_p_"+label+".png")
...
def greedy_global_linking(all_scores, score_cut=2.0, loose_cut=10):
    ...
    links_no_score = {}
#     print(all_scores)
    for score, itrk, its in all_scores:
        if score > loose_cut:
            break
...
def compute_score_block(
    ...
    for i in range(n):
        ...
        score_out[i] = wEp * rEp_norm*rEp_norm + (1-wEp) * pullR2
...
for ev in tqdm(prange(len(tracks))):
    ...
    links, links_no_score = greedy_global_linking(all_scores_ev, loose_cut=10)
#     print(links)
...
file1 = open("efficiencies.txt", "a")
file1.write(str(np.array([PT, ETA, effTight, effLoose, purity]))+'\n')
file1.close()
...
with open("missing_links_"+label+".txt", "w") as f:
    for ev, trk in wrong:
