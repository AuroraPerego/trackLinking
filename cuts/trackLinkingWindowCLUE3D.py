...
#            allsimtrackstersCP = load_branch_with_highest_cycle(file, 'ticlDumper/simtrackstersCP')
            ...
            allticlTrackster = load_branch_with_highest_cycle(filename, 'ticlDumper/ticlTrackstersCLUE3DHigh')
...
#         simtrackstersCP = allsimtrackstersCP.arrays(simTsKeys)
            ...
            tracksters3D = allticlTrackster.arrays(tsKeys)
        except:
            return []
    ...
    for ev in tqdm(prange(len(simtrackstersSC))):
        ...
        for idx in prange(len(stsSCEv.trackIdx)):
            ...
            ##########
            maskScore = assEv.ticlTrackstersCLUE3DHigh_simToReco_SC_score[idx] < 0.9
            tsEnergy = tsLinksEv.raw_energy[assEv.ticlTrackstersCLUE3DHigh_simToReco_SC[idx]]
            sharedEnergy = assEv.ticlTrackstersCLUE3DHigh_simToReco_SC_sharedE[idx]
            maskEnergy = sharedEnergy / tsEnergy > 0.4
            assIndices = assEv.ticlTrackstersCLUE3DHigh_simToReco_SC[idx]
    #         print(sharedEnergy[:3], sharedEnergy[:3] / tsEnergy[:3])
            maskIdx = assIndices[maskScore & maskEnergy]
    #         good = []
    #         for recoIdx in maskIdx:
    #             idx_in_r2s = np.where(idx==assEv.ticlTrackstersCLUE3DHigh_recoToSim_CP[recoIdx])[0][0]
    # #             print(np.where(idx==assEv.ticlTrackstersCLUE3DHigh_recoToSim_CP[recoIdx])[0][0])
    #             if assEv.ticlTrackstersCLUE3DHigh_recoToSim_CP_score[recoIdx][idx_in_r2s] < 0.6:
    #                 good.append(recoIdx)
...
if __name__ == "__main__":
    ...
    plt.tight_layout()
    plt.savefig(f"plotsWindow/3DcumulativeEnergyInWindow_200PU_pt{pt}_eta{eta}.png")
